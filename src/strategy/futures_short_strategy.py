...
class FuturesShortStrategy(BaseStrategy):
    def evaluate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            df['signal'] = signal
            df['entry'] = df[self.COL_CLOSE].where(signal, pd.NA)
            df['stop_loss'] = (df[self.COL_CLOSE] * (1 + self.stop_loss_pct)).where(signal, pd.NA)
            df['take_profit'] = (df[self.COL_CLOSE] * (1 - self.take_profit_pct)).where(signal, pd.NA)
            df['volume'] = df[self.COL_VOLUME].where(signal, pd.NA)
            df[self.COL_VOLUME_SCORE] = (abs(df[self.COL_PRICE_CHANGE]) * df[self.COL_VOLUME] * (100 - df[self.COL_RSI])).where(signal, pd.NA)

            df = df.dropna(subset=[self.COL_PRICE_CHANGE, self.COL_RSI])
